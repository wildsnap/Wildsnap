import { ChevronLeft, Trophy, Star, Lock, Camera, BookOpen } from "lucide-react";
...
export function AchievementsScreen({ 
  ...
  const getIcon = (title: string, isCompleted: boolean) => {
    if (!isCompleted) return <Lock className="w-6 h-6 text-[#2C2C2C] opacity-50" strokeWidth={3} />;
    if (title.toLowerCase().includes("scan")) return <Camera className="w-6 h-6 text-[#2C2C2C]" strokeWidth={3} />;
    if (title.toLowerCase().includes("story")) return <BookOpen className="w-6 h-6 text-[#2C2C2C]" strokeWidth={3} />;
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center h-full bg-[#E8F5E9] relative overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-[0.05]" style={{ backgroundImage: 'linear-gradient(#2C2C2C 2px, transparent 2px), linear-gradient(90deg, #2C2C2C 2px, transparent 2px)', backgroundSize: '20px 20px' }} />
    ...
    return (
      <div className="flex flex-col justify-center items-center h-full bg-[#E8F5E9] px-6 text-center relative overflow-hidden">
        ...
        <div className="border-4 border-[#2C2C2C] bg-white p-6 rounded-none shadow-[6px_6px_0_0_#2C2C2C] relative z-10">
          <p className="font-['Press_Start_2P'] text-sm text-[#FF4757] mb-4">GAME OVER</p>
          <p className="font-['Nunito'] font-bold text-[#2C2C2C] text-sm mb-6">{error}</p>
          <button 
            onClick={onRetry} // UPDATE 2: Now calls onRetry instead of onBack
  ...
  return (
    <div className="flex flex-col h-full bg-[#8B9BB4] relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-10" style={{ backgroundImage: 'linear-gradient(#000 2px, transparent 2px), linear-gradient(90deg, #000 2px, transparent 2px)', backgroundSize: '32px 32px' }} />
      ...
      <div className="bg-[#E2D8C3] border-b-4 border-[#2C2C2C] px-4 py-4 sticky top-0 z-20 shadow-[0_4px_0_0_rgba(0,0,0,0.2)] relative">
        <div className="flex items-center gap-4 max-w-md mx-auto">
          ...
          <div className="flex-1 flex justify-center items-center gap-3 pr-12">
            <Trophy className="w-6 h-6 text-[#FFC800] fill-[#FFC800] drop-shadow-[2px_2px_0_#2C2C2C]" />
        ...
        <div className="max-w-md mx-auto space-y-6">
          ...
          {achievements.map((achievement) => (
            ...
            >
              ...
              <div className="flex items-start gap-4">
                <div className={`shrink-0 w-16 h-16 border-4 border-[#2C2C2C] flex items-center justify-center shadow-[inset_-4px_-4px_0_0_rgba(0,0,0,0.2)] ${
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start mb-2">
                    <h2 className={`font-['Press_Start_2P'] text-[11px] leading-relaxed uppercase tracking-wider ${
                      achievement.isCompleted ? "text-[#2C2C2C]" : "text-[#4A4A4A]"
                    }`}>
                      {achievement.title}
                    </h2>
                  </div>
                  
                  <p className={`font-['Nunito'] text-sm font-black mb-3 line-clamp-2 ${
                    achievement.isCompleted ? "text-[#754F26]" : "text-[#4A4A4A]"
                  }`}>
                  ...
                  <div className="space-y-2 mt-auto">
                    <div className="flex justify-end">
                      <span className={`font-['Press_Start_2P'] text-[10px] ${
                        achievement.isCompleted ? "text-[#00D66F] drop-shadow-[1px_1px_0_#2C2C2C]" : "text-[#4A4A4A]"
                      }`}>
                    ...
                    <div className="w-full h-4 bg-[#2C2C2C] p-[2px]">
                      <div
                        ...
                        style={{
                          width: `${Math.min(
                            100,
                            (achievement.currentProgress / Math.max(1, achievement.targetValue)) * 100
                          )}%`,
                        }}
                      />
                    </div>
          ...
          {achievements.length === 0 && (
            <div className="text-center py-16">
              <span className="text-6xl block mb-4 drop-shadow-[4px_4px_0_rgba(0,0,0,0.2)]">👾</span>
